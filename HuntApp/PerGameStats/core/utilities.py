...
import numpy as np
from validation import GameBrId
...
def handle_err(e, game=None, games=None, additional_message=None):
    #########################################
...
def save_html(game, year):
    ...
    # four factors table
    if four_factors := re.findall(
        r'<table[^>]*id="four_factors"[^>]*>(.*?)</table>', str(soup), re.DOTALL
    ):
    ...
    # inactive players
    inactive_strong_tag = soup.find(
        "strong", string=re.compile(r"Inactive:", re.IGNORECASE)
    )
    if inactive_strong_tag:
        html += (
            '<div id="inactive_players">'
            + str(inactive_strong_tag.find_parent("div"))
            + "</div>"
        )
    else:
        with open("log.txt", "a+") as f:
    ...
    try:
        html += (
            soup.select(f"#box-{game['away_team_br_id']}-game-basic")[0].prettify()
            + "\n"
        )
    ...
    try:
        html += (
            soup.select(f"#box-{game['away_team_br_id']}-game-advanced")[0].prettify()
            + "\n"
        )
    except IndexError:
        pass
    ...
    try:
        html += (
            soup.select(f"#box-{game['home_team_br_id']}-game-basic")[0].prettify()
            + "\n"
        )
    except Exception as e:
        handle_err(e, game)
    ...
    try:
        html += (
            soup.select(f"#box-{game['home_team_br_id']}-game-advanced")[0].prettify()
            + "\n"
        )
    except IndexError:
        pass
    ...
    if year > 1995:
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-q1-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-q2-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-q3-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-q4-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-q1-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-q2-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-q3-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-q4-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-h1-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-h2-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-h1-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-h2-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            handle_err(e, game)
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-ot1-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-ot1-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-ot2-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-ot2-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-ot3-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-ot3-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-ot4-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-ot4-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-ot5-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-ot5-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['home_team_br_id']}-ot6-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
        ...
        try:
            html += (
                soup.select(f"#box-{game['away_team_br_id']}-ot6-basic")[0].prettify()
                + "\n"
            )
        except Exception as e:
            pass
...
def insertDataJL(fname, new_data):
    ...
    """
    new_data = [
        {k: int(v) if isinstance(v, np.int64) else v for k, v in item.items()}
...
def get_last_processed_game(table_name: str) -> GameBrId | None:
    ...
    try:
        ...
        handle_err(
            e,
            additional_message="ERROR DETERMINING THE LAST GAME PROCESSED (to pick up were the script left off)!",
        )
...
def is_same_season(game1: GameBrId | pd.Series, game2: GameBrId | pd.Series) -> bool:
    """
    Check if two games are in the same season.
    Args:
        game1 (GameBrId | pd.Series): The first game, either a BR ID or a pandas Series containing the game data.
        game2 (GameBrId | pd.Series): The second game, either a BR ID or a pandas Series containing the game data.
    Returns:
        bool: True if both games are in the same season, False otherwise.
    """
    if isinstance(game1, pd.Series):
        game1_br_id = game1["game_br_id"]
    else:
        game1_br_id = game1
    if isinstance(game2, pd.Series):
        game2_br_id = game2["game_br_id"]
    else:
        game2_br_id = game2
