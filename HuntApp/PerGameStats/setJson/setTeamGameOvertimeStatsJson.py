...
def setTeamGameOvertimeStatsJson(
    games,
    away_team_basic,
    home_team_basic,
    four_factors,
    ...
    file,
    year,
    accidently_set_team_br_id_backwards=True,
):
    """
    ...
    away_tgs["overtime"] = overtime
    home_tgs["overtime"] = overtime

    next_field = "minutes_played"
    away_tgs["minutes_played"] = away_team_basic.select('td[data-stat="mp"]')[
        -1
    ].text.strip()
    home_tgs["minutes_played"] = home_team_basic.select('td[data-stat="mp"]')[
        -1
    ].text.strip()
    ...
    away_tgs["field_goals"] = away_team_basic.select('[data-stat="fg"]')[
        -1
    ].text.strip()
    home_tgs["field_goals"] = home_team_basic.select('[data-stat="fg"]')[
        -1
    ].text.strip()

    next_field = "field_goal_attempts"
    away_tgs["field_goal_attempts"] = away_team_basic.select('[data-stat="fga"]')[
        -1
    ].text.strip()
    home_tgs["field_goal_attempts"] = home_team_basic.select('[data-stat="fga"]')[
        -1
    ].text.strip()

    next_field = "field_goal_percentage"
    away_tgs["field_goal_percentage"] = away_team_basic.select('[data-stat="fg_pct"]')[
        -1
    ].text.strip()
    home_tgs["field_goal_percentage"] = home_team_basic.select('[data-stat="fg_pct"]')[
        -1
    ].text.strip()

    next_field = "three_pointers"
    away_tgs["three_pointers"] = away_team_basic.select('[data-stat="fg3"]')[
        -1
    ].text.strip()
    home_tgs["three_pointers"] = home_team_basic.select('[data-stat="fg3"]')[
        -1
    ].text.strip()

    next_field = "three_pointer_attempts"
    away_tgs["three_pointer_attempts"] = away_team_basic.select('[data-stat="fg3a"]')[
        -1
    ].text.strip()
    home_tgs["three_pointer_attempts"] = home_team_basic.select('[data-stat="fg3a"]')[
        -1
    ].text.strip()

    next_field = "three_pointer_percentage"
    away_tgs["three_pointer_percentage"] = away_team_basic.select(
        '[data-stat="fg3_pct"]'
    )[-1].text.strip()
    home_tgs["three_pointer_percentage"] = home_team_basic.select(
        '[data-stat="fg3_pct"]'
    )[-1].text.strip()

    next_field = "free_throws"
    away_tgs["free_throws"] = away_team_basic.select('[data-stat="ft"]')[
        -1
    ].text.strip()
    home_tgs["free_throws"] = home_team_basic.select('[data-stat="ft"]')[
        -1
    ].text.strip()

    next_field = "free_throw_attempts"
    away_tgs["free_throw_attempts"] = away_team_basic.select('[data-stat="fta"]')[
        -1
    ].text.strip()
    home_tgs["free_throw_attempts"] = home_team_basic.select('[data-stat="fta"]')[
        -1
    ].text.strip()

    next_field = "free_throw_percentage"
    away_tgs["free_throw_percentage"] = away_team_basic.select('[data-stat="ft_pct"]')[
        -1
    ].text.strip()
    home_tgs["free_throw_percentage"] = home_team_basic.select('[data-stat="ft_pct"]')[
        -1
    ].text.strip()
    ...
    away_tgs["offensive_rebounds"] = away_team_basic.select('[data-stat="orb"]')[
        -1
    ].text.strip()
    home_tgs["offensive_rebounds"] = home_team_basic.select('[data-stat="orb"]')[
        -1
    ].text.strip()

    next_field = "defensive_rebounds"
    away_tgs["defensive_rebounds"] = away_team_basic.select('[data-stat="drb"]')[
        -1
    ].text.strip()
    home_tgs["defensive_rebounds"] = home_team_basic.select('[data-stat="drb"]')[
        -1
    ].text.strip()
    ...
    away_tgs["personal_fouls"] = away_team_basic.select('[data-stat="pf"]')[
        -1
    ].text.strip()
    home_tgs["personal_fouls"] = home_team_basic.select('[data-stat="pf"]')[
        -1
    ].text.strip()
